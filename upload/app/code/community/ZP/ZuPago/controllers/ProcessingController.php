<?php
class ZP_ZuPago_ProcessingController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    /**
     * Iframe page which submits the payment data to ZuPago HyBrid (HD) Wallet
     */
    ...
    /**
     * Show orderPlaceRedirect page which contains payment form
     */
    public function paymentAction()
    {
        try {
            ...
            $order->setState(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('zupago')->__('The customer was redirected to ZuPago HyBrid (HD) Wallet')
            );
            $order->save();
    ...
    {
        $event = Mage::getModel('zupago/event')
    ...
    public function cancelAction()
    {
        ...
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getZupagoQuoteId()) {
